...
import (
	...
	utils "github.com/crossplane-contrib/provider-kafka/internal/controller/namespaced/utils/acl"
	...
	v1alpha2 "github.com/crossplane-contrib/provider-kafka/apis/namespaced/acl/v1alpha2"
	apisv1alpha2 "github.com/crossplane-contrib/provider-kafka/apis/namespaced/v1alpha2"
)
...
// SetupGated adds a controller that reconciles AccessControlList managed resources with safe-start support.
func SetupGated(mgr ctrl.Manager, o controller.Options) error {
	o.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			panic(errors.Wrap(err, "cannot setup AccessControlList controller"))
		}
	}, v1alpha2.AccessControlListGroupVersionKind)
	return nil
}

func Setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1alpha2.AccessControlListGroupKind)
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1alpha2.AccessControlListList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1alpha2.AccessControlListList")
		}
	}

	r := managed.NewReconciler(mgr, resource.ManagedKind(v1alpha2.AccessControlListGroupVersionKind), opts...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		WithEventFilter(resource.DesiredStateChanged()).
		For(&v1alpha2.AccessControlList{}).
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	cr, ok := mg.(*v1alpha2.AccessControlList)
	if !ok {
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	compare := acl.CompareAcls(*extname, *utils.Generate(&cr.Spec.ForProvider))
	diff := acl.Diff(*extname, *utils.Generate(&cr.Spec.ForProvider))
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	generated := utils.Generate(&cr.Spec.ForProvider)
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	_, ok := mg.(*v1alpha2.AccessControlList)
	if !ok {
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	err := acl.Delete(ctx, c.kafkaClient, utils.Generate(&cr.Spec.ForProvider))
